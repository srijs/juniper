...
use ast::{Arguments, Document, Definition, Fragment, FragmentSpread, Selection, Field, Type};
...
impl<'a> OverlappingFieldsCanBeMerged<'a> {
        ...
        -> Vec<Conflict>
    {
        ...
        let (field_map, fragment_names) = self.get_fields_and_fragment_names(parent_type, selection_set, ctx);
    ...
    )
    {
        ...
        {
            if self.compared_fragments.borrow().contains(fragment_name1, fragment_name2, mutually_exclusive) {
                return;
            }
        }

        {
            self.compared_fragments.borrow_mut().insert(fragment_name1, fragment_name2, mutually_exclusive);
        }

        let (field_map1, fragment_names1) = self.get_referenced_fields_and_fragment_names(fragment1, ctx);
        let (field_map2, fragment_names2) = self.get_referenced_fields_and_fragment_names(fragment2, ctx);
    ...
    )
    {
        ...
        let (field_map2, fragment_names2) = self.get_referenced_fields_and_fragment_names(fragment, ctx);
                ...
                return Some(Conflict(
                    ConflictReason(
                        ...
                        ConflictReasonMessage::Message(format!(
                            "{} and {} are different fields", name1, name2))),
                ...
                return Some(Conflict(
                    ConflictReason(
                        ...
                        ConflictReasonMessage::Message(
                            "they have differing arguments".to_owned())),
                    vec![ast1.start.clone()],
                    vec![ast2.start.clone()]));
            }
        }
            ...
            if self.is_type_conflict(ctx, t1, t2) {
                return Some(Conflict(
                    ConflictReason(
                        response_name.to_owned(),
                        ConflictReasonMessage::Message(format!(
                            "they return conflicting types {} and {}", t1, t2))),
                    vec![ast1.start.clone()],
                    vec![ast2.start.clone()]));
            }
        }
        ...
        -> Vec<Conflict>
    {
        ...
        let (field_map1, fragment_names1) = self.get_fields_and_fragment_names(parent_type1, selection_set1, ctx);
        let (field_map2, fragment_names2) = self.get_fields_and_fragment_names(parent_type2, selection_set2, ctx);
        ...
        -> Option<Conflict>
    {
        ...
        Some(Conflict(
            ...
            vec![pos1.clone()].into_iter()
                .chain(conflicts.iter().flat_map(|&Conflict(_, ref fs1, _)| fs1.clone()))
                .collect(),
            vec![pos2.clone()].into_iter()
                .chain(conflicts.iter().flat_map(|&Conflict(_, _, ref fs2)| fs2.clone()))
                .collect()))
    }
        ...
        -> bool
    {
        match (args1, args2) {
            (&None, &None) => true,
            (&Some(Spanning { item: ref args1, .. }), &Some(Spanning { item: ref args2, .. })) => {
        ...
        -> (AstAndDefCollection<'a>, Vec<&'a str>)
    {
        ...
        self.collect_fields_and_fragment_names(parent_type, selection_set, ctx, &mut ast_and_defs, &mut fragment_names);
...
impl<'a> Visitor<'a> for OverlappingFieldsCanBeMerged<'a> {
    ...
    fn enter_selection_set(&mut self, ctx: &mut ValidatorContext<'a>, selection_set: &'a Vec<Selection>) {
        for Conflict(ConflictReason(reason_name, reason_msg), mut p1, mut p2)
            in self.find_conflicts_within_selection_set(ctx.parent_type(), selection_set, ctx)
        {
            p1.append(&mut p2);
...
fn error_message(reason_name: &str, reason: &ConflictReasonMessage) -> String {
    ...
    format!(r#"Fields "{}" conflict because {}. {}"#, reason_name, format_reason(reason), suffix)
}

fn format_reason(reason: &ConflictReasonMessage) -> String {
    match *reason {
        ConflictReasonMessage::Message(ref name) => name.clone(),
        ConflictReasonMessage::Nested(ref nested) =>
            nested.iter()
...
mod tests {
    ...
    fn same_aliases_with_different_field_targets() {
        ...
        "#,
            &[
                RuleError::new(
                    &error_message("fido", &Message("name and nickname are different fields".to_owned())), &[
    ...
    fn alias_masking_direct_field_access() {
        ...
        "#,
            &[
                RuleError::new(
                    &error_message("name", &Message("nickname and name are different fields".to_owned())), &[
    ...
    fn different_args_second_adds_an_argument() {
        ...
        "#,
            &[
                RuleError::new(
                    &error_message("doesKnowCommand", &Message("they have differing arguments".to_owned())), &[
    ...
    fn conflicting_return_types_which_potentially_overlap() {
        expect_fails_rule_with_schema(QueryRoot, factory, r#"
            {
              someBox {
                ...on IntBox {
                  scalar
    ...
    fn compatible_return_shapes_on_different_return_types() {
        expect_passes_rule_with_schema(QueryRoot, factory, r#"
          {
            someBox {
              ... on SomeBox {
                deepBox {
